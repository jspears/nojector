...
_u.extend(Injector.prototype, {
    ...
    stringify: function (obj, ctx, promises) {
        promises = promises || [];
        var oret, ret, oret = ret = {}, p = promise(), self = this;
        JSON.stringify(obj, function (k, v) {
            ...
            if (isPromise(v)) {
                promises.push(p.then(v.then(function (res) {
                    return self.stringify(res, ctx)
                })));
                return null;
            }
            if (typeof v === 'function') {
                promises.push(p.next(self.resolve(v, this, ctx).then(function (res) {
                    return self.stringify(res, ctx).then(function (v) {
                        ret[k] = v;
                        return v;
                    });
                })));
