...
__.extend(Injector.prototype, {
    ...
    stringify: function (obj, ctx) {
        return promise().when(this._stringify(obj, ctx)).then(function (args) {
            return args[0]
        });
    },
    /**
    ...
    _stringify: function (obj, ctx) {
        ...
        switch (typeof obj) {
            ...
            case "number":
                return obj;
            case "function":
                return self.resolve(obj, null, ctx).next(function (vv) {
                    return self._stringify(vv, ctx);
                });
            default:
            {
                if (obj == null)
                ...
                return promise().when(__.map(obj, function (v, k) {
                    var resolve = resolveF(ret, k);

                    if (typeof v === 'function') {
                        return self.resolve(v, obj, ctx).next(function (vv) {
                            var r = self._stringify(vv, ctx);
                            if (isPromise(r)) {
                        ...
                        var r = self._stringify(v, ctx);
                        if (isPromise(r)) {
                            return r.next(resolve);
                        } else {
                            resolve(r);
                            return false;
                        }
