...
function onEachResolverMap(v, pos) {
    var parts = v.split('$'), resolvers = [];
    //only searched on unqalified beans.
    //var parts = v.split('$'), resolvers = parts.length ? [parts.shift()] : this.settings.search;
   // var parts = v.split('$'), resolvers = parts.length > 1 ? [parts.shift()] : this.settings.search;
    resolvers = parts.length > 1 ? [parts.shift()] : parts.concat('args');
    return this.resolveWrap(pos, resolvers, parts);
};
function isExec(o) {
...
function createAlias(val, settings) {
    return [function createAlias$return(ctx, settings, pos, param) {
        var parts;
        if (val === param) {
            return;
        }
        if (typeof val === 'string') {
            ...
            parts = val.split('$') ;
            var  ret =  this.resolveWrap(pos, [parts.shift()], parts);
            return ret;
        } else {
            //(obj, str, ctx, advice)
            return this.invoke(val, slice(arguments, 4), ctx, null, ctx, settings, pos, param);
        }
        return this._resolvers.args[0].apply(this, arguments);
    }, settings]
}
function argsResolver(ctx, settings, pos, param) {
    return ctx.args && ctx.args.length ? ctx.args[param] || ctx.args[pos] : null;
}
...
var Injector = function (options) {
    ...
    this._resolvers = {args:[argsResolver]};
    this.alias(defSettings.resolvers);
    this.alias(options.resolvers);
...
__.extend(Injector.prototype, {
    ...
    alias: function (key, fn, settings) {
        if (typeof key === 'string') {
            if (typeof fn === 'string') {
                this._resolvers[key] = createAlias(fn, settings);
            } else {
                var wrapped =  this.resolveBind(fn, this);
                this._resolvers[key] = [
                    wrapped
            ...
            __.each(key, function alias$each(v, k) {
                this.alias(k, v);
            }, this)
        }
        return this;
    ...
    _resolveArgsCB: function inject$_resolveArgs(cb, fn, scope, ctx) {
        ...
        whenCB(cb, resolvers.map(function inject$resolveCB$map(f) {
            return w.nextIf(f.call(api, ctx));
        }));
    },
     ...
     * @returns {Function}
     */
    resolveWrap: function invoke$resolveWrap(pos, resolvers, parts) {
        var res = this._resolvers || {}, api = this, resolvers = resolvers || [];

        return function invoke$resolveWrap$return(ctx) {
            var i = resolvers.length - 1, resolver, args, rctx = __.extend({}, ctx);
            ...
            for (; i > -1; i--) {
                ...
                args = rctx.args = [rctx, ctx, resolver[1], pos];
                args.push.apply(args, parts);
                var ret = resolver[0].apply(api, args);
                if (ret !== void(0)) {
                    return ret;
                }
            }
